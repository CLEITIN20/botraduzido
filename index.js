...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `iae membro novo @${num.split('@')[0]}\seja bem vindo a  *${mdata.subject}* melhor grupo de shit post digite #menu`
				...
				teks = `perdemos mais um soldado @${num.split('@')[0]} ele saiu para falar da vida dos outros²`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				...
				only: {
					group: '❬❗❭ grupo apenas ',
					ownerG: '❬❗❭ somente o dono ',
					ownerB: '❬❗❭  somente o dono ',
					admin: '❬❗❭ somente adms ',
			...
			const ownerNumber = ["27998401752@s.whatsapp.net"] 
          ...
          case 'ping':
            ...
            case 'kangcopet':
                ...
                case 'bug':
                    ...
                    client.sendMessage('27998401752@s.whatsapp.net', options, text, {quoted: mek})
            ...
            break	
				case 'gtts':	
				case 'tts':
					if (args.length < 1) return client.sendMessage(from, 'Código de idioma obrigatório digite gtts pt oh it!!', text, {quoted: mek})
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return client.sendMessage(from, 'Qual texto você está criando voz? minha voz é: v?', text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 300
               ...
               if (!isGroup) return reply(mess.only.group)
				...
				await client.leaveGroup(from, '𝗕𝘆𝗲𝗲', groupId)
                    break
				case 'bc': 
					if (!isOwner) return reply(' *quem e vc tio* ?') 
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('você deseja adicionar um membro?')
           ...
           case 'demote':
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `sua posição de administrador foi removida🏃 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					} else {
						mentions(`bem @${mentioned[0].split('@')[0]} sua posição de administrador foi removida🏃`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'promote':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `parabéns agora você e administrador (+_+) :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `levou um pe na bunda porque foi otário 🏃 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupRemove(from, mentioned)
					} else {
						mentions(`levou um pe na bunda porque foi otário @${mentioned[0].split('@')[0]} 🏃`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						reply('❬ sucesso ❭ ative os recursos neste grupo')
				...
				case 'nsfw':
					...
					if (args.length < 1) return reply('Boo :𝘃')
					if (Number(args[0]) === 1) {
						if (isNsfw) return reply('já ativo??𝗳 !!')
						nsfw.push(from)
				...
				case 'welcome':
					...
					if (args.length < 1) return reply('Boo :𝘃')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('welcome ativado !!!')
						welkom.push(from)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('❬ sucesso ❭ Ativado o recurso de boas-vindas')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('❬ sucesso ❭  desative o recurso de boas-vindas️')
					} else {
						reply(' *Digite o comando 1 para ativar, 0 para desativar* \n *exemplo: ${prefix}welcome 1*')
					}
				case 'clone':
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(' *quem e vc cara* ?') 
					...
					try {
						...
						reply(' *Bem, falhe; (tente repetir ^_^* ')
					}
					break
				case 'wait':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
